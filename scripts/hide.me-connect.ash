#!/bin/ash
# Using a previously obtained access token, query a specific hide.me VPN server with a freshly generated wireguard key-pair
# to create a connection-unique wireguard config, setup the wireguard interface, back up the resolv.conf, configure DNS routing
# to use the wireguard DNS, and connect.
# Dev Note: the generated wireguard config includes a SessionKey value for the disconnect script included as a comment.
#
# Be sure to use the hide.me-disconnect.ash! The resolv.conf for DNS routing is restored by it, and the generated wireguard
# config is immediately deactivated by it rather than it counting against your device limit until it gets timed out by the server.
#
# Args:
#   None
# Env:
#   HIDE_ME_SERVER: Required. The subdomain of hideservers.net for the server to connect to.
#   HIDE_ME_TOKEN_FILE: Optional. The access token file generated by hide.me-accessToken.ash. Contains a base64 encoded access token.
#                       Default=./accessToken.txt
#   HIDE_ME_CONF_PATH: Optional. The folder to store the generated wireguard config in. Default=/tmp
#   HIDE_ME_DEV_NAME: Optional. Name to use for the wireguard interface. Default=hide.me
# Example Usage:
